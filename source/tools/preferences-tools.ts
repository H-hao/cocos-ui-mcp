...
export class PreferencesTools implements ToolExecutor {
    ...
    private async setPreferencesConfig(category: string, path: string, value: any, scope: string = 'global'): Promise<ToolResponse> {
        return new Promise((resolve) => {
            ...
            (Editor.Message.request as any)('preferences', 'set-config', trimmedCategory, trimmedPath, value, scope).then((success: boolean) => {
                if (success) {
                    resolve({
                        success: true,
    ...
    private async resetPreferencesConfig(category: string, scope: string = 'global'): Promise<ToolResponse> {
        return new Promise((resolve) => {
            ...
            (Editor.Message.request as any)('preferences', 'query-config', trimmedCategory, undefined, 'default').then((defaultConfig: any) => {
                if (!defaultConfig) {
                    throw new Error(`No default configuration found for category "${trimmedCategory}"`);
                }
                // Apply default configuration
                return (Editor.Message.request as any)('preferences', 'set-config', trimmedCategory, '', defaultConfig, scope);
